...
const userValidations = [
	check('firstName')
		...
		.bail()
		.isLength({ min: 2 })
		.withMessage('Tiene que ser mayor a 2 caracteres')
		.bail()
		.isLength( { max: 100 })
		.withMessage('Tiene que ser menor a 100 caracteres'),
	check('lastName')
		.notEmpty()
	...
	check('municipio')
		...
		.bail()
		.isLength( { max: 100 })
		.withMessage('Tiene que ser menor a 100 caracteress'),
	check('ciudad')
		.notEmpty()
	...
	check('numeroVivienda')
		...
		.custom((value, {req}) => {
			if (value < 0) {
				throw new Error ('No hay viviendas negativas')
			}
	...
	check('codigoPostal')
		...
		.custom((value, {req}) => {
			if (value < 0) {
				throw new Error ('No hay codigos postales negativos')
			}
